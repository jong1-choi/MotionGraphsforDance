...
int playBack() {
    ...
    ma_engine_play_sound(&engine, "/Users/choijongwon/Downloads/featureData/Deep Silencio - Underwater.wav", NULL);
...
//            if(int(interval)+1 == m.frameNum)
//                b.add(l.parentIndex, l.childIndex, a * (l.l) + (1-a) * (m.bodies[int(interval)].links[j].l), glm::slerp(l.q, m.bodies[int(interval)].links[j].q, 1-a));
//            else
//                b.add(l.parentIndex, l.childIndex, a * (l.l) + (1-a) * (m.bodies[int(interval)+1].links[j].l), glm::slerp(l.q, m.bodies[int(interval)+1].links[j].q, 1-a));
            j++;
        }
...
//    std::cout << m.bodies.size()/30 << " " << duration << std::endl;
    return m;
}
