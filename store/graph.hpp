...
//    vector<Node*> findNodeVec(Edge* e){ return nodeMap.find(e)->second; }
...
//            std::cout << "total frame -> "<< n->endFrame - n->startFrame << std::endl;
            std::cout << std::endl;
        }
    }
...
//void constructNode(MotionGraph &g, int windowSize, const vector<Motion> &motions) {
//    for(int i = 0; i < motions.size(); i++) {
//        for(int j = windowSize/2; j < motions[i].frameNum - windowSize/2; j++){
//            for(int k = j + windowSize+1; k < motions[i].frameNum - windowSize/2; k++){
//                if( !motions[i].bodies[j].isBeat || !motions[i].bodies[k].isBeat) continue;
//                Node *n = new Node(i, j, k, g.findEdge(i,j), g.findEdge(i,k) );
//                g.addNodeList(*n);
//            }
//        }
//    }
