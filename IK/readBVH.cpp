...
void  BVH::UpdatePose( Joint * joint, const double * data, Body &body, float scale)
{
    ...
    for ( int i=0; i < joint->channels.size(); i++ )
    {
        ...
        if ( channel->type == X_ROTATION )
            ...
            joint->yRot = float(data[channel->index])*PI/180/2;
        }
    ...
    if ( parent == NULL )
    {
        ...
        body.add(-1, joint->children[0]->index, joint->position, joint->quat, joint->yRot, vec3(0), quat(1,0,0,0), joint->has_site, isHand);
    }
        ...
        body.add(parent->index, -1, offset, joint->quat, joint->yRot, parent->position, parent->quat, joint->has_site, isHand);
    }
    else
    {
        glm::vec3 offset = scale * glm::vec3(joint->offset[ 0 ], joint->offset[ 1 ], joint->offset[ 2 ]);
        ...
        body.add(parent->index, joint->children[0]->index, offset, joint->quat, joint->yRot, parent->position, parent->quat, joint->has_site, isHand);
    }
